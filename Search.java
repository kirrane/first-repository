...
public class Search{
	public static void main(String[] args){
		...
		//message promt to enter search value from array
		...
		//creates int from array for column row
		for(int i = 0; i < dataSet.length; i++){
			for(int j = 0; j < dataSet[i].length; j++){

					//if search is correct will print match from array
					if(searchStr.equals(dataSet[i][j])){
						match++;
						if(match == 1){

						}
						//Print to show each item on menu associated with row in the array
							System.out.println();
		...
		//Provides Space for neetness
		System.out.println();

		//Prints out number of matchs with the search
		System.out.println("Records with match: "+match);

		//links file to main menu named Index
		Index.main(null);
